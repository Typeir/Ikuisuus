...
const TRIGGER_MAP: Record<string, string> = {
  action: 'action', bonus_action: 'bonus', reaction: 'reaction', passive: 'none', free: 'none',
};
...
function activation(type: string) {
  return { type, value: type !== 'none' ? 1 : null, condition: '', override: false };
}
...
function buildFlags(feature: MonsterFeature): Record<string, unknown> {
  return { 'ikuisuus-damocles': { featureId: feature.id, ...(feature.meta ?? {}) } };
}
...
function buildUses(feature: MonsterFeature): Record<string, unknown> {
  ...
  return {
    spent: 0, max: '1',
    recovery: [{ period: 'recharge', formula: r.min === r.max ? `${r.min}` : `${r.min}-${r.max}` }],
  };
}
...
function toDamageField(formula: string, type: string): DamageField {
  ...
  return {
    number: p?.count ?? null, denomination: p?.sides ?? null, bonus: p?.bonus ?? '',
    types: type ? [type] : [], custom: { enabled: !p, formula: p ? '' : formula },
  };
}
 ...
 */
export function buildWeaponItem(feature: MonsterFeature, itemId: string): FoundryItem {
  ...
  const activities: Record<string, FoundryActivity> = {
    dnd5eactivity000: createAttackActivity({
      ability: atk.type === 'ranged' ? 'dex' : 'str', bonus: String(atk.bonus), flat: true,
      attackType: atk.type === 'ranged' ? 'ranged' : 'melee', classification: 'weapon',
      includeBase: true, base: { activation: activation(actType) },
    }),
  };
  if (feature.saving_throw) {
    const fp = feature.damageFlat ? parseDamageFormula(feature.damageFlat) : null;
    activities['dnd5eactivity100'] = createSaveActivity({
      id: 'dnd5eactivity100', ability: feature.saving_throw.ability,
      dcFormula: String(feature.saving_throw.dc), onSave: 'half',
      damageParts: fp ? [createDamagePart(fp.count, fp.sides, feature.damageFlatType ?? '')] : [],
    });
  }
  const system: Record<string, unknown> = {
    description: { value: feature.description ?? '' }, type: { value: 'natural', baseItem: '' },
    damage: {
      base: feature.damage ? toDamageField(feature.damage, feature.damageType ?? '') : EMPTY_DAMAGE_FIELD,
      versatile: EMPTY_DAMAGE_FIELD,
    },
    range: { value: atk.reach ?? null, long: atk.range?.long ?? null, units: 'ft', reach: atk.reach ?? null },
    properties: [], proficient: 1, equipped: true, identifier: toIdentifier(feature.name), activities,
  };
  if (feature.recharge) system.uses = buildUses(feature);
  return { _id: itemId, name: feature.name, type: 'weapon', system, flags: buildFlags(feature) };
}
 ...
 */
export function buildSaveFeatItem(feature: MonsterFeature, itemId: string): FoundryItem {
  const save = feature.saving_throw!;
  const dp: ReturnType<typeof createDamagePart>[] = [];
  if (feature.damage) {
    const p = parseDamageFormula(feature.damage);
    if (p) dp.push(createDamagePart(p.count, p.sides, feature.damageType ?? ''));
  }
  const system: Record<string, unknown> = {
    description: { value: feature.description ?? '' }, type: { value: 'monster', subtype: '' },
    identifier: toIdentifier(feature.name),
    activities: {
      dnd5eactivity000: createSaveActivity({
        ability: save.ability, dcFormula: String(save.dc), onSave: 'half', damageParts: dp,
  ...
  if (feature.recharge) system.uses = buildUses(feature);
  if (feature.target)
    system.target = { value: feature.target.range ?? null, units: 'ft', type: feature.target.type ?? '' };
  return { _id: itemId, name: feature.name, type: 'feat', system, flags: buildFlags(feature) };
}
 ...
 */
export function buildMultiattackItem(feature: MonsterFeature, itemId: string): FoundryItem {
  const attacks = feature.multiattack!.attacks
    .map((a) => `${a.count} ${a.name}${a.count > 1 ? ' attacks' : ' attack'}`).join(', ');
  const actType = resolveActivation(feature);
  return {
    _id: itemId, name: feature.name, type: 'feat',
    system: {
      description: { value: `<p>The creature makes ${attacks}.</p>` },
      type: { value: 'monster', subtype: '' }, identifier: toIdentifier(feature.name),
      activities: {
        dnd5eactivity000: createUtilityActivity({
          base: { activation: activation(actType !== 'none' ? actType : 'action') },
        }),
      },
    },
 ...
 * @param {string} itemId - Generated Foundry document ID
 * @returns {FoundryItem} Feat item with no activities
 */
export function buildPassiveItem(feature: MonsterFeature, itemId: string): FoundryItem {
  const system: Record<string, unknown> = {
    description: { value: feature.description ?? '' }, type: { value: 'monster', subtype: '' },
    identifier: toIdentifier(feature.name), activities: {},
  };
  if (feature.recharge) system.uses = buildUses(feature);
  return { _id: itemId, name: feature.name, type: 'feat', system, flags: buildFlags(feature) };
}
