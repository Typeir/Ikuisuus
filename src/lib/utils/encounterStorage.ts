...
import type { AffixEntry, CreatureEntry, CreatureStats, Encounter } from '@/lib/types/encounterPlanner';
...
export const getEncounters = (): Encounter[] => {
  ...
  try {
    ...
    logger.warning('Error loading encounters from localStorage', { error: error instanceof Error ? error.message : String(error) });
    return [];
  }
};
...
export const saveEncounters = (encounters: Encounter[]): void => {
  ...
  try {
    ...
    logger.error('Error saving encounters to localStorage', { error: error instanceof Error ? error.message : String(error), encounterCount: encounters.length });
  }
};
 ...
 */
export const createCreatureFromMonster = (monsterData: any, locale: string = 'en'): CreatureEntry => {
  ...
  const sourceHref = monsterData.link ? `/${locale}${monsterData.link}` : undefined;
