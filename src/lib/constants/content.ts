...
/** Regex matching any content-type suffix, derived from {@link CONTENT_SUFFIXES}. */
export const REGEX_CONTENT_SUFFIX = new RegExp(
  `\\.(${CONTENT_SUFFIXES.map((suffix) => suffix.slice(1)).join('|')})$`,
);
