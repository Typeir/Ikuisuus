 ...
 */
interface Notification extends Required<Pick<NotificationConfig, 'id' | 'type' | 'message' | 'duration' | 'dismissible'>> {
  title?: string;
...
interface NotificationContextValue {
  ...
  success: (message: ReactNode, options?: Partial<NotificationConfig>) => string;
  /** Shorthand for warning notification */
  warning: (message: ReactNode, options?: Partial<NotificationConfig>) => string;
...
export const NotificationProvider = memo(function NotificationProvider({
   ...
   */
  const push = useCallback((config: NotificationConfig): string => {
  ...
  const info = useCallback((message: ReactNode, options?: Partial<NotificationConfig>) => 
    push({ ...options, message, type: 'info' }), [push]);
  
  const success = useCallback((message: ReactNode, options?: Partial<NotificationConfig>) => 
    push({ ...options, message, type: 'success' }), [push]);
  
  const warning = useCallback((message: ReactNode, options?: Partial<NotificationConfig>) => 
    push({ ...options, message, type: 'warning' }), [push]);
  
  const error = useCallback((message: ReactNode, options?: Partial<NotificationConfig>) => 
    push({ ...options, message, type: 'error' }), [push]);
...
const NotificationItem = memo(function NotificationItem({
  ...
  const getIcon = () => (
    <svg className={styles.icon} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
      <path fillRule="evenodd" d={NOTIFICATION_ICON_PATHS[notification.type ?? 'info']} clipRule="evenodd" />
    </svg>
  );
