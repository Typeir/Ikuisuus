 ...
 *
 * @requires vitest
 * @requires @/app/api/spells/[slug]/route
 */

import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import * as SpellSlugRoute from '@/app/api/spells/[slug]/route';

const { GET } = SpellSlugRoute;

describe('Single Spell API Route', () => {
  beforeEach(() => {
  ...
  describe('Module exports', () => {
    ...
    it('should only export expected handlers', () => {
      const exports = Object.keys(SpellSlugRoute);
  ...
  describe('GET handler behavior', () => {
    it('should return a Response object with valid slug', async () => {
      const request = new Request('http://localhost/api/spells/test-spell');
      const context = { params: Promise.resolve({ slug: 'test-spell' }) };
    ...
    it('should handle locale parameter in URL', async () => {
      const request = new Request('http://localhost/api/spells/test-spell?locale=en');
      const context = { params: Promise.resolve({ slug: 'test-spell' }) };
      const response = await GET(request, context);
      expect(response).toBeInstanceOf(Response);
    });
    ...
    it('should return spell object or error in body', async () => {
      const request = new Request('http://localhost/api/spells/test-spell');
      const context = { params: Promise.resolve({ slug: 'test-spell' }) };
      const response = await GET(request, context);
    ...
    it('should return 404 for non-existent spell slug', async () => {
      const request = new Request('http://localhost/api/spells/definitely-not-a-real-spell-12345');
      const context = { params: Promise.resolve({ slug: 'definitely-not-a-real-spell-12345' }) };
      const response = await GET(request, context);
      expect([404, 500]).toContain(response.status);
    });

    it('should handle async params pattern (Next.js 15)', async () => {
      const request = new Request('http://localhost/api/spells/test');
