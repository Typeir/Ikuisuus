 ...
 * @requires @/lib/mdx/rehypeSectionize - Plugin under test
 */
 ...
 *
 * @param {string} html - Input HTML fragment
 * @param {import('@/lib/mdx/rehypeSectionize').RehypeSectionizeOptions} opts - Plugin options
 * @returns {string} Serialized HTML output
 */
function processWithOptions(
  html: string,
  opts: import('@/lib/mdx/rehypeSectionize').RehypeSectionizeOptions,
): string {
  const tree = fromHtml(html, { fragment: true });
