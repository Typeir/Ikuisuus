 ...
 *
 * @module tests/unit/src/modules/library/presentation/components/slots/feature/Feature.test
 * @version 0.2.0
 * @author Typeir
 * @since 2026-09-02
 */
...
import Feature, {
  ...
  Curse,
  Trait,
} from '@/modules/library/presentation/components/slots/feature/Feature';
import {
  Cost,
  Targets,
...
describe('Feature', () => {
  ...
  it('reads a paragraph of slot elements the same way', () => {
    ...
    expect(article?.querySelector('h6 [data-feature-cost]')?.textContent).toBe(
      '1 Minor Action',
    );
    expect(article?.querySelector('[data-slot="targets"]')).toBeTruthy();
    expect(article?.querySelector('[data-slot="cost"]')).toBeNull();
  ...
  it('marks a deed as a deed when it states no cost', () => {
    const { container } = render(
      <Feature deed='phase'>
        <h4>Long Night</h4>
        <p>Body.</p>
  ...
  it('keeps the mark of the block around it when it only recharges', () => {
    const { container } = render(
  ...
  it('a feature is a plain article carrying its anchor, with no section of its own', () => {
    ...
    expect(container.querySelector('h4')).toHaveAttribute(
      'data-anchor',
      'vanish',
    );
...
describe('Attack', () => {
  it('prints accuracy and reach as slot lines under the heading, and the hit as prose', () => {
    ...
    expect(
      container.querySelector('h5 [data-heading-title]')?.textContent,
    ).toBe('Lash');
    expect(
      container.querySelector('[data-slot="accuracy"] [data-slot-value]')
        ?.textContent,
    ).toBe('+7');
    expect(
      container.querySelector('[data-slot="reach"] [data-slot-value]')
        ?.textContent,
    ).toBe('[= 3 stride =]');
    expect(container.querySelector('[data-feature-body]')?.textContent).toBe(
      'On a hit, 13.',
    );
  });
  ...
  it('renders without a heading inside a bare action, range and targets as lines', () => {
    ...
    expect(
      container.querySelector('[data-slot="range"] [data-slot-value]')
        ?.textContent,
    ).toBe('[= 6 stride =]');
    expect(
      container.querySelector('[data-slot="targets"] [data-slot-value]')
        ?.textContent,
    ).toBe('up to two targets');
  });
  ...
  it('folds into a details block when collapsible, keeping the heading as the summary', () => {
    ...
    expect(
      container.querySelector('details [data-feature-body]')?.textContent,
    ).toBe('Prose body.');
  });
  ...
  it('keeps its heading and shows the grid outside any details block when not collapsible', () => {
    ...
    expect(
      container.querySelector('h6 [data-heading-title]')?.textContent,
    ).toBe('Extended Reach');
  });
