...
vi.mock('@/modules/world-sim/shaders/collisionCloud.vert.glsl', () => ({
  default: 'void main() { gl_Position = vec4(0.0); }',
}));
vi.mock('@/modules/world-sim/shaders/collisionCloud.frag.glsl', () => ({
  default: 'void main() { gl_FragColor = vec4(1.0); }',
}));
vi.mock('@/modules/world-sim/shaders/collisionCore.vert.glsl', () => ({
  default: 'void main() { gl_Position = vec4(0.0); }',
}));
vi.mock('@/modules/world-sim/shaders/collisionCore.frag.glsl', () => ({
  default: 'void main() { gl_FragColor = vec4(1.0); }',
}));
...
describe('CollisionCloudEffect', () => {
  ...
  it('core fades through alpha as the explosion dissipates', async () => {
    const { APEX_TIME, FADE_DURATION } =
      await import('@/modules/world-sim/infrastructure/effects/collisionCloudLayers.core');
    effect = new CollisionCloudEffect(PAIR_ID);
    const a = new Vector3(0, 0, 0);
    const b = new Vector3(SUM_R, 0, 0);
