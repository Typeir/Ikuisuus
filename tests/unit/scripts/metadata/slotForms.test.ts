...
describe('unslotBloodline', () => {
  it('returns each boon to a collapsible and blanks the rest, keeping line count', () => {
    const source =
    ...
    expect(text).toContain('</Collapsible>');
    expect(text).not.toMatch(/<\/?(?:Bloodline|Boons|Feature)\b/);
    // a plain core feature wrapper is blanked, not turned into a collapsible
    expect(text).toContain('### Languages');
    // the slot elements stay, since the generator reads the values off them
    expect(text).toContain('<Senses>Darkvision</Senses>');
