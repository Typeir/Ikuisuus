 ...
 *
 * @param {string} componentsFile - Path to mdxComponents.tsx
...
async function loadAutoGeneratedComponents(
  ...
  await readComponentsFile(componentsFile, /mdxComponents\["(\w+)"\]/g);
  return names;
}
 ...
 *
 * @param {string} indexFile - Path to index.tsx
...
async function loadKnownComponents(
  ...
  const registered = await loadRegisteredComponents(indexFile);
  const autoGenerated = await loadAutoGeneratedComponents(componentsFile);
...
export async function runCheck(options?: CheckOptions): Promise<CheckResult> {
  ...
  const mdxComponentsFile = path.join(
    rootDir,
    'src',
    'lib',
    'components',
