...
const MDX_COMPONENTS_FILE = path.join(ROOT, 'src', 'lib', 'components', 'mdx', 'mdxComponents.tsx');
const MDX_INDEX_FILE = path.join(ROOT, 'src', 'lib', 'components', 'mdx', 'index.tsx');
...
const RULES: FormatRule[] = [
  {
    ...
    suggestion: 'Rename to kebab-case: lowercase letters, numbers, and hyphens only',
    severity: 'critical',
  },
  {
  ...
  },
  {
    ...
    suggestion: 'Register the component in src/lib/components/mdx/index.tsx or use a registered component',
    severity: 'critical',
  },
  {
    name: 'missing-h1',
  ...
  },
  {
    ...
    suggestion: 'Use CSS variables via className instead of inline color styles',
    severity: 'warning',
  },
  {
  ...
  },
  {
    ...
    message: 'Isolated empty blockquote line (not part of multi-line blockquote)',
    suggestion: 'Remove orphaned > lines or connect them to adjacent blockquote content',
    severity: 'warning',
  },
  {
  ...
  },
  {
    ...
    check: (content: string, rel: string) =>
      rel.endsWith('.sheet.mdx') && !content.match(/\|\s*\*{0,2}STR\*{0,2}\s*\|/),
  ...
  },
  {
    ...
    suggestion: 'Spell files should contain a blockquote (>) section with the spell stat block',
    severity: 'warning',
    appliesTo: ['spells'],
  },
];
...
async function loadAutoGeneratedComponents(): Promise<Set<string>> {
  ...
  const readComponentsFile = async (filePath: string, pattern: RegExp): Promise<void> => {
    try {
  ...
  await readComponentsFile(MDX_INDEX_FILE, /^\s+(\w+)(?:\s*[:,]|\s*:\s)/gm);
  return names;
}
...
function getIgnoredRules(content: string): Set<string> {
  ...
  const htmlPattern = /<!---?\s*health:check-ignore\s+([a-z0-9-]+?)(?:\s*)--+>/gi;
 ...
 */
async function findMdxFiles(dir: string, results: string[] = []): Promise<string[]> {
  let entries;
  try {
