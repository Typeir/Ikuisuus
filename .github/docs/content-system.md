...
└── en/                           # English content (only locale with content)
    ...
    │   └── heirlooms/
    │       ├── main.mdx
    │       └── [item].mdx       # Individual heirloom files
    │       └── [item].metadata.json  # Auto-generated
    ├── monsters/
    │   ├── main.mdx
...
- **Extensions**: `.mdx` (standard), `.sheet.mdx` (monster stat blocks)
...
function mergeLocale(locale) {
  ...
  const files = fs.readdirSync(localeDir).filter(f => f.endsWith('.json') && f !== 'index.json');
