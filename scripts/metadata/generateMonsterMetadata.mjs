...
function parseItalicMeta(lines, sharedData = null) {
  ...
  creatureType = creatureType.replace(/^\s*,?\s*/g, '').replace(/\(.*?\)/g, '').trim();
...
function findArmorHpSpeed(lines) {
  ...
  const hpAverage = hpMatch ? ParsingUtils.parseNumericValue(hpMatch[1]) : undefined;
  const hpFormula = hpMatch && hpMatch[2] ? hpMatch[2] : undefined;

  return {
    ac: ac !== undefined ? { value: ac, notes: acNotes, raw: acRaw } : undefined,
    hp: hpAverage !== undefined ? { average: hpAverage, formula: hpFormula, raw: hpRaw } : undefined,
...
function parseSpeed(raw) {
  ...
  for (const p of parts) {
    ...
    const m = p.match(/(?:(walk|climb|fly|swim|burrow|land)\s+)?(\d+)\s*ft\.?/i);
    if (m) {
...
function parseAbilities(lines) {
  ...
  const idx = lines.findIndex(l => /^\s*\|\s*\*?\*?STR\*?\*?\s+\|\s*\*?\*?DEX\*?\*?\s+\|\s*\*?\*?CON\*?\*?\s+\|\s*\*?\*?INT\*?\*?\s+\|\s*\*?\*?WIS\*?\*?\s+\|\s*\*?\*?CHA\*?\*?\s*\|/i.test(l));
...
function findStatBlockPositions(lines, sharedData = null) {
  ...
  const blockquoteStatBlockPattern = new RegExp(`^>\\s*_(${sizePattern})\\b`, 'i');
 ...
 */
function extractStatBlockSection(lines, statBlockLineIdx, endIdx, isBlockquote) {
...
function findStatBlockTitle(allLines, statBlockLineIdx, isBlockquote) {
  ...
  for (let i = statBlockLineIdx - 1; i >= Math.max(0, statBlockLineIdx - 15); i--) {
 ...
 */
function parseStatBlockSection(allLines, sectionLines, statBlockLineIdx, isBlockquote, baseSlug, filePath, statBlockIndex, sharedData = null) {
  ...
  const nonmagicalPattern = /bludgeoning,?\s+piercing,?\s+and\s+slashing\s+from\s+nonmagical\s+[^;,]+/i;
  const resist = ParsingUtils.splitListWithGrouping(bulletMap['Damage Resistances'], nonmagicalPattern);
  const immune = ParsingUtils.splitListWithGrouping(bulletMap['Damage Immunities'], nonmagicalPattern);
  const vuln = ParsingUtils.splitListWithGrouping(bulletMap['Damage Vulnerabilities'], nonmagicalPattern);
  const condImm = ParsingUtils.splitList(bulletMap['Condition Immunities']);
  const skills = bulletMap['Skills'] ? ParsingUtils.splitList(bulletMap['Skills']) : [];
  const cr = parseCR(bulletMap['Challenge']);
  const proficiencyBonus = parseProficiencyBonus(bulletMap['Proficiency Bonus']);
  
  // Extract tags: main stat block gets entire file, nested blocks get their section only
  const linesToScan = (statBlockIndex === 0 && !isBlockquote) ? allLines : sectionLines;
  ...
  if (contentStartIdx === 0) {
    for (let i = 0; i < linesToScan.length; i++) {
      ...
      if (/^[-*]\s+\*\*(Proficiency Bonus|Challenge|Languages|Senses|Condition Immunities|Damage|Skills|Saving Throws)\*\*/i.test(line)) {
        contentStartIdx = i + 1;
      }
    }
  }
  ...
  if (!italicMeta.size) {
    log.debug('Missing size field', { creature: displayName, file: baseSlug, nested: isNested });
  }
  if (!italicMeta.creatureType) {
    log.debug('Missing creature type', { creature: displayName, file: baseSlug, nested: isNested });
  }
  if (!cr) {
    log.debug('Missing CR', { creature: displayName, file: baseSlug, nested: isNested });
  }
  if (!headerStats.ac) {
    log.debug('Missing AC', { creature: displayName, file: baseSlug, nested: isNested });
  }
  if (!headerStats.hp) {
    log.debug('Missing HP', { creature: displayName, file: baseSlug, nested: isNested });
  }
  if (!headerStats.speed) {
    log.debug('Missing speed', { creature: displayName, file: baseSlug, nested: isNested });
  }
  ...
  return {
    ...
    link: subSlug !== baseSlug ? `/library/monsters/${baseSlug}#${subSlug}` : `/library/monsters/${baseSlug}`,
...
async function parseMonsterFile(filePath, sharedData = null) {
  ...
  if (statBlockPositions.length > 1) {
    log.debug('Multi-stat block file detected', { file: baseSlug, statBlockCount: statBlockPositions.length });
  }
  ...
  for (let i = 0; i < statBlockPositions.length; i++) {
    ...
    const sectionLines = extractStatBlockSection(lines, lineIndex, endIdx, isBlockquote);
    const metadata = parseStatBlockSection(lines, sectionLines, lineIndex, isBlockquote, baseSlug, filePath, i, sharedData);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().catch(error => {
    log.error('Fatal error during monster metadata generation', { error: error.message, stack: error.stack });
    process.exit(1);
  });
}
