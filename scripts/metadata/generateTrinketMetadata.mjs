...
function parseTrinketProperties(content, sharedData) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('**Properties**:')) {
      ...
      if (propsMatch) {
        ...
        const cleanedText = propsText.replace(/special\s*\([^)]+\)/gi, 'special');
  ...
  for (const condition of conditionKeywords) {
    ...
    const patterns = [
      new RegExp(`\\b(become|becomes|is|are|fall|falls|be)\\s+${condition}\\b`, 'i'),
  ...
  if (inflictedConditions.length > 0) {
    ...
    result.inflictsConditions = inflictedConditions.map(c => c.split(',').map(part => part.trim()).join(', '));
  }
...
async function parseTrinketFile(filePath, sharedData) {
  try {
    ...
    const metadata = {
      ...
      ...properties,
      tags: TaggingUtils.extractAllTags(raw, filePath, sharedData, { contentType: 'trinket' })
    };
    
    return metadata;
    
  } catch (error) {
    log.warning('Error parsing trinket file', { file: filePath, error: error.message });
    return null;
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().catch(error => {
    log.error('Fatal error during trinket metadata generation', { error: error.message, stack: error.stack });
    process.exit(1);
  });
}
