...
function parseComponents(lines) {
  ...
  const content = componentLine.replace(/^>\s*\*\*Components\*\*:\s*/i, '').trim();
...
function parseSpellProperties(lines) {
  ...
  // Casting Time
  const castingTimeLine = lines.find(l => /^>\s*\*\*Casting Time\*\*:/i.test(l));
  if (castingTimeLine) {
...
function parseCastingTimeToArray(rawCastingTime) {
  ...
  if (/\b(?<!bonus\s)action\b/i.test(lowerText) && !castingTimes.includes('bonus action')) {
  ...
  const timeMatch = rawCastingTime.match(/(\d+\s*(?:minute|min|hour|hr|round|day)s?)/i);
  if (timeMatch) {
...
function generateSpellTags(fullText, metadata, sharedData) {
  ...
  for (const ability of abilities) {
    const savePattern = new RegExp(`\\b${ability.long}\\s+saving\\s+throw\\b`, 'i');
...
function parseSpellLists(content) {
  ...
  const spellListsMatch = content.match(/####\s*Spell Lists\s*([\s\S]*?)(?=\n#|$)/i);
...
async function parseSpellFile(filePath, sharedData) {
  ...
  // Generate tags
  const tags = generateSpellTags(content, { ...headerData, ...properties, ...components }, sharedData);
...
async function generateSpellMetadata(options = {}) {
  ...
  const externalMetadataPath = path.join(process.cwd(), 'scripts', 'core', 'spells-external.metadata.json');
  
  try {
    const stats = await fs.stat(externalMetadataPath);
    if (stats.isFile()) {
      log.message('Found external spell metadata', { path: externalMetadataPath });
      ...
      const spellsFolder = path.join(process.cwd(), 'src', 'content', 'en', 'spells');
      
      // Copy the entire external metadata file as-is to the spells folder
      const destinationPath = path.join(spellsFolder, 'spells-external.metadata.json');
      await fs.writeFile(destinationPath, JSON.stringify(externalSpells, null, 2), 'utf8');
      
      log.message('Copied external spell metadata', { spellCount: externalSpells.length, destination: destinationPath });
    }
  } catch (err) {
    // External metadata file doesn't exist, that's okay
    if (err.code !== 'ENOENT') {
      log.warning('Error checking for external spell metadata', { error: err.message });
    }
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  generateSpellMetadata().catch(error => {
    log.error('Fatal error during spell metadata generation', { error: error.message, stack: error.stack });
    process.exit(1);
  });
}

export { generateSpellMetadata as main, generateSpellMetadata, parseSpellFile };
