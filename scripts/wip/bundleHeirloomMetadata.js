...
async function bundleHeirloomMetadata() {
  ...
  const outputFile = path.join(process.cwd(), 'src', 'lib', 'data', 'heirlooms.json');
  ...
  try {
    ...
    const allHeirlooms = metadataFiles.map(file => {
    ...
    console.log(`✓ Bundled ${allHeirlooms.length} heirlooms to ${outputFile}`);
  } catch (error) {
    console.error('Error bundling heirloom metadata:', error);
    process.exit(1);
  }
}
