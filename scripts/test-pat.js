...
console.log(
  'Using GITHUB_PAT from',
  process.env.GITHUB_PAT ? 'process.env' : envFromFile ? envPath : 'unknown',
);
  ...
  res.on('end', () => {
    try {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        ...
        if (acceptedHeader)
          console.log('Accepted scopes for endpoint:', acceptedHeader);
        ...
        if (needsRepoScope) {
          if (!hasRepo) {
            ...
            console.error(
              'Regenerate the PAT with the `repo` scope (or use a token with org SSO approved access).',
            );
            process.exit(2);
          }
        } else {
          if (!hasRepo && !hasPublicRepo) {
            ...
            console.error(
              'Regenerate the PAT with `public_repo`, or `repo` for full access.',
            );
            process.exit(2);
          }
        }
